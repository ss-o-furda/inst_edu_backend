...
sudo apt-get install -y -q python3-pip python3.10-dev python3.10-venv
...
chmod 777 /apps
mkdir /apps/logs
mkdir /apps/logs/instedu_api
mkdir /apps/logs/instedu_api/app_log
setfacl -m u:apiuser:rwx /apps/logs/instedu_api
...
#Clone repo
cd /apps
git clone https://github.com/ss-o-furda/inst_edu_backend.git

# Setup the web app:
cd /apps/inst_edu_backend
...
cp /apps/inst_edu_backend/app/server/nginx/instedu.nginx /etc/nginx/sites-enabled/
...
certbot --nginx -d instedu-api.orikfw.com